...
int datadd(struct blackbook *record)
{
      if (fsptr)
      {
            ...
            record->delete_flag = 0; /* active record tag */
            record->recordnum = (int) (ftell(fsptr) / 
                  (long) sizeof(struct blackbook));

            if (fwrite(&record, sizeof(struct blackbook), 1, fsptr) != 1)
            {
...
int data_read(long recnum)
{
      ...
      else                          /* copy save struct to edit struct */
      {
			...
			printf("oldrec.num : %d, fread.num : %d, %s\n", oldrec.recordnum, rec.recordnum, rec.firstname);            
            return Success_;
      }
}
