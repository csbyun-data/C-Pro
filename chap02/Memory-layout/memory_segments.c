// [book] Pointers in C  A Hands on Approach.pdf
#include <stdio.h>
#include <malloc.h>

int glb_uninit; // Part of BSS Segment -- global uninitized variable, at runtime it is initialized to zero
...
void foo(void) {
  ...
  printf("Address of heap allocated insize foo() %p\n", ptr_foo);
  foo();  // function call
}
