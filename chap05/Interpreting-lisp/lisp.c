/* Filename:  ~\lisp\lisp.c            Revision Date: Feb. 16, 2017 */
/*****************************************************************************
...
/* The above includes declare strlen(), strcpy(), strcmp(), calloc(),
   ...
   register-save structure template: jmp_buf is defined. This include 
   header-file  will need to be constructed to conform to any particular 
   system. */
...
char getgchar(void)
/*----------------------------------------------------------------------------
 Fill the buffer string g and set pg=pointer to g (if needed), and then 
 remove and return the next character from the input found at pg and 
...
{while (pg>=pge)
   {sprompt: if (filep EQ stdin) {sprintf(sout,"%c",prompt); ourprint(sout);}
     if (fgetline(g,200,filep)<0)  /* -1 result means EOF */
        {*g = '\0'; pg= g; pge= g; return;}
...
/* -----------------------
    / the problem is when / xxxxx is the last line!  /

    /debug!!!!!/ else { ourprint(":"); ourprint(g); ourprint("&\n");
                          ourprint("g[0]");
                          sprintf(sout,"%c", *g); 
                          ourprint(sout);
                          if (*g EQ '/') ourprint("> *g = /\n");       
...
#define CR  13
#define LF  10
 for (i=0; i<lim AND (c=fgetc(stream))!=EOF AND c!= LF; ++i)
   {if ((c EQ TAB) OR (c EQ CR)) c= BLANK; s[i]= c;}
 s[i]= '\0';
...
/* int32 v; a typed-pointer to the object to be printed
...
#define Return(v) {traceprint(v,1); return(v);}
 ...
 if (NOT builtin(ty))
	...
	while (p!=nilptr AND dottedpair(type(fa)))
     ...
     if (p!=nilptr) error("too many actual argumentss");
     /* The following code would forbid some useful trickery. 
        if (fa!=nilptr) error("too many formal arguments"); */
