...
char tempMap[14][56]; // Map

char map1[14][56] = { // Easy Map
...
void drawMap(int* x, int* y) {
  ...
  for(h=0; h<14; h++) { 
    for(w=0; w<56; w++) {
      ...
      if(temp == '0'){ // Empty Area
        setColor(black, black);  
        printf(" "); 
      } else if(temp == '1') { // Wall(#)
        setColor(white, white);  
        printf("#"); 
      } else if(temp == 's') { // Player(@) 
        *x = w; // 플레이어 좌표값 저장  
        *y = h;
        setColor(cyan, black);  
        printf("@");
      } else if(temp == 'q') { // End (O)
        setColor(lightgreen, black);    
        printf("O"); 
      } else if(temp == 'k') { // Key(*) 
        setColor(yellow, black);
        printf("*"); 
      } else if(temp == 'l') { // Close Door(+) 
...
void drawUI(int* x, int* y, int* key){
  setColor(white, black); // Basic Color 
  gotoxy(3, 16);
  printf("Position: %02d, %02d", *x, *y); // x, y 값 출력 
...
void infoDraw(){
  system("cls");
  printf("\n\n");
  printf("                 [ A method of operation ]\n\n");
  printf("                    Move: W, A, S, D\n");
  printf("                    Choose: Space Bar\n\n\n\n\n\n\n");
  printf("               Developer: codevkr.tistory.com\n\n");
