...
static struct relation *rfind(char *rname);
static void seek(struct scan *sptr, unsigned int tnum);
static void make_fname(char *fname,char *rname);
...
static struct relation *rfind(char *rname)
{
    ...
    if ((rptr = (struct relation *)malloc(sizeof(struct relation))) == NULL) {
        close(fd);
  ...
  char *rname;
{
    ...
    if ((sptr = (struct scan *)malloc(sizeof(struct scan))) == NULL)
        return (db_nerror(INSMEM));

    /* allocate a tuple buffer */
    if ((sptr->sc_tuple = (char*)malloc(rptr->rl_size)) == NULL) {
        free(sptr);
        return (db_nerror(INSMEM));
    }
