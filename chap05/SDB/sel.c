...
/* SEL.C */
static get_sattrs ( struct sel *slptr );
static get_srels ( struct sel *slptr );
static srelation ( struct sel *slptr , char *rname , char *aname );
static int check_attrs ( struct sel *slptr );
static int all_attrs ( struct sel *slptr );
static int find_attr(struct sel *slptr, char *rname, char* aname, char **paptr, struct srel **psrel, struct attribute **pattr);
static int uattr ( struct sel *slptr , char *aname , char **paptr , struct srel **psrel , struct attribute **pattr );
static int qattr ( struct sel *slptr , char *rname , char *aname , char **paptr , struct srel **psrel , struct attribute **pattr );
static int process ( struct srel *srptr );
...
//        db_scan(fmt,a1,a2,a3,a4,a5,a6,a7,a8,a9);
    ...
    if ((slptr = (struct sel *)malloc(sizeof(struct sel))) == NULL)
  ...
  struct sel *slptr; char *rname,*aname,*avalue;
{
    ...
    if ((newbd = (struct binding *)malloc(sizeof(struct binding))) == NULL)
...
static get_sattrs(struct sel *slptr)
{
    ...
    while (TRUE) {
        ...
        if ((newsattr = (struct sattr *)malloc(sizeof(struct sattr))) == NULL)
            return (db_ferror(INSMEM));
        ...
        if ((newsattr->sa_aname = (char*)malloc(strlen(dbv_tstring)+1)) == NULL) {
            free(newsattr);
            return (db_ferror(INSMEM));
        }
        ...
        if (db_token() == ID) {
            ...
            if ((newsattr->sa_name = (char*)malloc(strlen(dbv_tstring)+1)) == NULL) {
...
static srelation(struct sel *slptr, char *rname,char *aname)
{
    ...
    if ((newsrel = (struct srel *)malloc(sizeof(struct srel))) == NULL)
        return (db_ferror(INSMEM));
    ...
    if (aname != NULL) {
        ...
        /* allocate space for the alternate name */
        if ((newsrel->sr_name = (char*)malloc(strlen(aname)+1)) == NULL) {
            free(newsrel);
            return (db_ferror(INSMEM));
        }
...
static int all_attrs(struct sel *slptr)
{
    ...
    for (srptr = slptr->sl_rels; srptr != NULL; srptr = srptr->sr_next) {
        ...
        for (i = 0; i < NATTRS; i++) {
            ...
            if ((newsattr->sa_rname = (char*)malloc(RNSIZE+1)) == NULL) {
                free(newsattr);
                return (db_ferror(INSMEM));
            }
            ...
            /* save the attribute name */
            if ((newsattr->sa_aname = (char*)malloc(ANSIZE+1)) == NULL) {
                free(newsattr->sa_rname);
                free(newsattr);
                return (db_ferror(INSMEM));
            }
