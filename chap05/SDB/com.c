...
static int operator ( int (*opr )(void));
static int operand ( struct operand *opr );
static int expr ( int *result );
static int land ( int *result );
static int relat ( int *result );
static int primary ( int *result );
static int factor ( int *result );
static int get_operand ( int *result );
static int get_attr ( int *result );
static int get_number ( int *result );
static int get_string ( int *result );
static void freelit ( union codecell *cptr );
  ...
  int *result;
{
    ...
    if ((opr->o_value.ov_char.ovc_string =
                (char*)malloc(strlen(dbv_tstring)+1)) == NULL) {
        free(opr);
        return (db_ferror(INSMEM));
    }
