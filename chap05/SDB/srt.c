...
static struct skey *get_skeys ( struct scan *sptr );
static void free_skeys ( struct skey *skeys );
static int find_attr ( struct scan *sptr , struct skey *newskey , char *aname );
static int sort ( struct skey *skeys , struct scan *sptr1 , struct scan *sptr2 , struct scan *sptr3 );
static int compare ( struct skey *skeys , struct scan *sptr1 , struct scan *sptr2 );
static int cattr ( struct skey *cskey , struct scan *sptr1 , struct scan *sptr2 );
static int assign ( struct scan *sptr1 , struct scan *sptr2 );
...
static struct skey *get_skeys(struct scan *sptr )
{
    ...
    while (TRUE) {
        ...
        if ((newskey = (struct skey *)malloc(sizeof(struct skey))) == NULL)
