...
static int compare ( const int cmp );
static int comp ( struct operand *lval , struct operand *rval );
static int ncomp ( struct operand *lval , struct operand *rval );
static int boolean ( const int opr );
...
static int boolean(const int opr)
{
    ...
    if ((result = (struct operand *)malloc(sizeof(struct operand))) == NULL)
